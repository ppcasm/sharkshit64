...
bool unbond_ble_device(void) {
   ...
   if (esp_ble_get_bond_device_list(&dev_num, bonded_dev_list) == ESP_OK && dev_num > 0) {
       ...
       if (err == ESP_OK) {
           ...
           ESP_LOGE(BLE_HOST_TAG, "Failed to remove bonded device: %s", esp_err_to_name(err));
       }
   }
   return false;
}
...
void hidh_callback(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
   ...
   case ESP_HIDH_OPEN_EVENT: {
       if (param->open.status == ESP_OK) {
           ...
           ESP_LOGE(BLE_HOST_TAG, "Failed to open device (status: 0x%02x)", param->open.status);  
   ...
   if (dev_num > 0) {
       ...
       if (esp_ble_get_bond_device_list(&dev_num, bonded_dev_list) == ESP_OK) {
           ...
           while (!ble_connected) {
               ...
               if (!ble_connected) {
                   ESP_LOGE(BLE_HOST_TAG, "Bonded connection failed. Retrying in 1s...");
